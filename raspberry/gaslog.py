...
import raspberry.hmc5883l as hmc
import argparse
import requests

URL='https://lzue8n.deta.dev/counter'
...
#trigger_level = 1900
#trigger_hyst = 400
#for angle is more save, since independent auf amplitude
trigger_level = 0
trigger_hyst = 130
...
def main():
  ...
  while(1==1):
    ...
    (bx,by,bz,nonecount)=compass.safeaxes()
    ...
    ba = math.atan2(bx,by)*180/math.pi # angle x,y (to be used later)
    if ba > trigger_level + trigger_hyst:
       trigger_state = 1
    elif ba < trigger_level - trigger_hyst:
         trigger_state = 0
    if old_state == 0 and trigger_state == 1:
      ...
      print('%s: %s'%(nowstr,update))
      rrdtool.update(count_rrd, update)
      timestamp = time.time()
      PARAMS=dict(counter=counter,tick=1)
      data=dict(nix=0)
      start=time.time()
      r=requests.post(url = URL, data = data, params=PARAMS)
      timeused=time.time()-start
      print('time for request:%f s'%timeused)
      ...
      rrdtool.update(count_rrd, update)
      timestamp = time.time()
      PARAMS=dict(counter=counter,tick=0)
      data=dict(nix=0)
      start=time.time()
      r=requests.post(url = URL, data = data, params=PARAMS)
      timeused=time.time()-start
